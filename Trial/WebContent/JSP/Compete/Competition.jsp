...
<%
	...
	DBObject ParticipatingUsers = new BasicDBObject();
	if(doc.containsKey("ParticipatingUsers"))
	{
		ParticipatingUsers= (DBObject)doc.get("ParticipatingUsers");
		if(ParticipatingUsers.containsKey(username))
			out.print("<script>alert('"+"You can only take part in the competition once."+"'); location='AllCompetitions.jsp';</script>");
	}
...
function before_submitting(){
	var a=confirm('Still some time is left, Do you want to Submit?');
	if(a== false)
		return false;
	else
		{
			document.getElementById("body").style.display="none";
			document.getElementById("beforesubmitting").style.display= "block";
			return true;
		}
}	
</script>
</head>
<body onload="setStartTime();checkforlogin()">
	<%-- <div>
				...
				var x = setInterval(function() {
				  ...
				  if (distance < 0) {
				    ...
				    document.getElementById("demo").innerHTML = "EXPIRED";
				  	document.getElementById("compe").submit();
				  }
				}, 1000);
				</script>
   	</div>
   	...
   	<form id="compe" action="SubmitCompetition.jsp?id=<%= Cid %>" method="post">
   	<h3><%=doc.get("Name") %>  <span><button class="button" onclick="return before_submitting();">Submit</button></span></h3>
				...
				<tr style="border-bottom: thin;">
					<td style="padding:2px;padding-top: 10px;text-align: left;">Q.<%=i+1 %>)</td>
					<td style="padding: 2px;padding-top: 10px;text-align: left;"><button onclick="return show('<%=qid[i]%>');setPlaceholder();" style="background: none; border: none;font: inherit;width:100%;text-align:left; ;"><%=doc1.get("Name")%></button></td>
				</tr>
				<%} %>
   					</table>
   				</div>
			...
			for(int i=0;i<qid.length;i++)
			{
   			...
   			%>
   				<div id="<%=qid[i]%>">
    					...
    					<button name="compileandrun<%=qid[i] %>" class="button" style="width: 150px;float: left;" onclick="return take_values('<%=qid[i] %>')">Compile & Run</button>
    					<button name="SubmitProblem<%=qid[i] %>" class="button" style="width: 150px ;float: right;" onclick="return submit_values('<%=qid[i] %>')" >Submit Problem</button>
    					<br style="clear: both;" /><br/>
    					<textarea id="idsub<%=qid[i] %>" rows="5" cols="115" disabled="disabled" >Output will Be Shown Here</textarea>
    					<textarea id="SubmissionResult<%=qid[i] %>" rows="5" cols="115" disabled="disabled" >Submission Result will Be Shown Here</textarea>
    				...
    				<!-- <button name="Submit" class="button" style="float: right;" onclick="submit_values()" >Submit</button>
    				 -->
   			   	...
   			   	<%}
   			   		out.print("<script>for(var i=0;i<qid.length;i++){if(qid[i] != '')document.getElementById(qid[i]).style.display='none';}document.getElementById('"+qid[0]+"').style.display='block';</script>");
   	...
   	<div style="position:fixed;bottom:0px;overflow: hidden;background:white;border-radius:0px;margin-left:-8px;margin-top:-8px;width: 101.05%;color: black;height: 40px;box-shadow: 0px 0px 16px 0px rgba(0,0,0,0.2);">
		...
		<p style="display: none;" id="beforesubmitting">Please Wait While Your Solution is being Submitted.</p>
</body>
</html>